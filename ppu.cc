...
using std::array;
constexpr array<RgbColor, 4> palette = { 0xc4f0c2, 0x5ab9a8, 0x1e606e, 0x2d1b00 };
// constexpr array<RgbColor, 4> palette = { 0xffffff, 0xb6b6b6, 0x676767, 0x000000 };

static inline u8
translate_color(std::pair<Color, Color_Palette> p)
{
    u16 palette_register;
    switch (p.second) {
    case Color_Palette::BG: palette_register = reg_bgp; break;
    case Color_Palette::OBP0: palette_register = reg_obp0; break;
    case Color_Palette::OBP1: palette_register = reg_obp1; break;
    }

    assert(p.first < 4);
    return (mem.get(palette_register) & (0b11 << (2 * p.first))) >> (2 * p.first);
}

array<RgbColor, 8>
render_tile_line(const array<std::pair<Color, Color_Palette>, 8>& line)
{
    array<RgbColor, 8> result;

    for (int i = 0; i < 8; i++) result[i] = palette[translate_color(line[i])];
    return result;
}

array<RgbColor, 160>
render_line(const array<std::pair<Color, Color_Palette>, 160>& line)
{
    array<RgbColor, 160> result;
    for (int i = 0; i < 160; i++) { result[i] = palette[translate_color(line[i])]; }
    return result;
}

array<std::pair<Color, Color_Palette>, 8>
get_tile_line(const Memory& mem, u16 tile_addr, u8 line, Color_Palette palette)
{
    array<std::pair<Color, Color_Palette>, 8> result;
...
array<std::pair<Color, Color_Palette>, 160>
lcdc_bg_line(u8 lcdc, u8 top_pixel, u8 left_pixel)
{
    array<std::pair<Color, Color_Palette>, 160> result;
    if (!get_bit(lcdc, 0))
        std::fill(result.begin(), result.end(), std::pair<Color, Color_Palette>(0, Color_Palette::BG));
    else {
        auto bg_tile_map = get_bit(lcdc, 3) ? 0x9c00 : 0x9800;
        for (int ix = 0; ix < 32; ix++) {
            u16 offset = 32 * std::floor(top_pixel / 8) + (left_pixel + ix) % 32;
            u8 chr = mem.get(bg_tile_map + offset);
            u16 tile_addr = get_bit(lcdc, 4) ? 0x8000 + 16 * chr : 0x9000 + 16 * (s8)chr;
            ((array<std::pair<Color, Color_Palette>, 8>*)result.data())[ix] =
                get_tile_line(mem, tile_addr, top_pixel % 8, Color_Palette::BG);
        }
    }
    return result;
}
...
auto
lcdc_window_line(u8 lcdc, u16 top_pixel, u16 left_pixel, array<std::pair<Color, Color_Palette>, 160>& line) -> void
{
    ...
    for (int ix = 0; ix < 32; ix++) {
        ...
        s8 chr = (s8)mem.get(window_tile_map + offset);
        u16 tile_addr = 0x9000 + 16 * chr;
        auto window_line = get_tile_line(mem, tile_addr, top_pixel % 8, Color_Palette::BG);
        ((array<std::pair<Color, Color_Palette>, 8>*)line.data())[ix] = window_line;
    }
}
...
lcdc_render_sprite(u8 sprite_size, u8 sprite_idx, u8 iy)
{
    ...
    array<std::pair<Color, Color_Palette>, 8> line;
    auto tile_y = get_bit(sprite.flags, Object::Y_FLIP) ? (-iy) % sprite_size : iy % sprite_size;
    auto palette = get_bit(sprite.flags, Object::OBP1) ? Color_Palette::OBP1 : Color_Palette::OBP0;
    line = get_tile_line(mem, tile_addr, tile_y, palette);
                    ...
                    array<std::pair<Color, Color_Palette>, 160>& line) -> array<std::pair<Color, Color_Palette>, 160>
{
    ...
    for (int i = sprite_ids.second - 1; i >= 0; i--) {
        ...
        sprite.x -= 8;
        u16 tile_addr = 0x8000 + 2 * sprite_size * sprite.tile;
        auto tile_y = (iy - sprite.y) % sprite_size;
        if (get_bit(sprite.flags, Object::Y_FLIP))
            tile_y = sprite_size - tile_y;

        array<std::pair<Color, Color_Palette>, 8> tile_line;
        auto palette = get_bit(sprite.flags, Object::OBP1) ? Color_Palette::OBP1 : Color_Palette::OBP0;
        tile_line = get_tile_line(mem, tile_addr, tile_y, palette);
        ...
        if (!get_bit(sprite.flags, Object::BG_PRIORITY)) {
            ...
            for (auto j = 0; j < 8; j++) {
                ...
                if (line[sprite.x + j].second == Color_Palette::BG && line[sprite.x + j].first)
                    continue;
                line[sprite.x + j] = tile_line[j];
            }
        }
